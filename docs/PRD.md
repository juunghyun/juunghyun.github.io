...
flowchart LR
  ...
  B --> C[윤문<br/>AI 티 제거]
  C --> D[영어 번역<br/>초벌 + 본인 검수]
  D --> E[발행 체크리스트]
  E --> F[PR 머지<br/>동시 발행]
  F --> G[배포<br/>LinkedIn 요약 + 원문 링크]
```

소재는 일하다 마주친 문제를 그날 이슈로 남기는 데서 시작한다. 번역은 Claude Code로 초벌을 만들고, 코드·수치·고유명사를 대조한 뒤 본인이 끝까지 읽고 고친다.
...
|  | 테마 유지 | 유지 비용 0 | 개성 없음 |  |
| 번역 검수 | 본인 검수만 | 빠름 | 어색한 표현 놓침 | 본인 검수 + AI 교차 확인 |
|  | AI 교차 확인 추가 | 용어·수치 누락 잡음 | 한 단계 늘어남 |  |
