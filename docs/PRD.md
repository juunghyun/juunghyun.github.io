...
flowchart LR
  ...
  B --> C[윤문<br/>AI 티 제거]
  C --> D[영어 번역<br/>Claude 초벌·윤문, 본인 승인]
  D --> E[발행 체크리스트]
  E --> F[PR 머지<br/>동시 발행]
  F --> G[배포<br/>LinkedIn 요약 + 원문 링크]
```

소재는 일하다 마주친 문제를 그날 이슈로 남기는 데서 시작한다. 번역은 Claude Code가 쓴다(2026-09-22 결정, 21일 결정을 뒤집음). AI 티가 나지 않는 친근하고 읽기 편한 영어로 옮기고, 본인이 읽고 승인한 뒤 발행한다.

| 주차 | 할 일 | 산출물 |
| --- | --- | --- |
| 1주 | 소재 고르기, 한국어 초안, 코드 실행 확인 | 한국어 초안, 검증된 코드 |
| 2주 | 윤문, 영어 번역(Claude)과 본인 승인, OG 확인, PR | 영어판, PR 머지, 동시 발행 |
...
|  | 테마 유지 | 유지 비용 0 | 개성 없음 |  |
| 번역 방식 | 결정: Claude Code가 친근한 영어로 옮기고 본인이 승인 (2026-09-22, 21일 결정을 뒤집음) | 편당 시간 절약, 영어 품질 일정 | AI 티 위험, 자기 문장에서 멀어질 수 있음 | AI 티 없는 윤문을 조건으로, 본인 승인 필수 |
...
| 추가 | 번역 방식 변경 (2026-09-22) | Claude Code가 AI 티 없는 친근한 영어로 번역, 본인 승인 | 3.3, 9절, PLAN 1·5절 |
